"""Merge candidate table-scan poses with previous component entries (selective preserve)."""
...
def merge_scan_into_components(
    ...
    # Non-preserved catalog entries from this scan pass
    for tid, entry in cand_in.items():
        if tid in preserve and tid in prev_in:
            ...
            # Preserve requested but no prior row (e.g. first scan): use candidate
            merged[tid] = json.loads(json.dumps(entry))

    # Preserved-only keys (unexpected but safe): keep snapshot
    for tid in preserve:
