...
def _paths_from_request_backend() -> Optional["LabViewPaths"]:
    if _backend_registry_ref is None:
        return None
    try:
        from lab_model.coordinator.backends.dispatch import get_request_backend_id
...
def get_lab_view_paths() -> LabViewPaths:
    from lab_model.coordinator.backends.context import get_context_paths
...
def get_lab_manifest() -> LabViewManifest:
    from lab_model.coordinator.backends.context import get_context_manifest
...
def _load_lab_manifest(paths: LabViewPaths, project_root: str) -> LabViewManifest:
    ...
    if not os.path.isfile(p):
        ...
        doc: Dict[str, Any] = {
            ...
            "description": "Which edge host class this bundle targets (mock | real).",
