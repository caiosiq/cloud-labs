...
from lab_model.language.domain.component import get_measurables, get_tunables, normalize_components_map
...
class LabPose:
    ...
    def from_entry(cls, entry: Dict[str, Any]) -> "LabPose":
        ...
        :data:`lab_model.language.domain.holding.BREADBOARD_SURFACE_Z_LAB_MM`
        (not ``0`` — floor level would forward-transform to a dangerously
        low robot ``z``).
        """
        from lab_model.language.domain.component import presence_of, PRESENCE_BREADBOARD
        from lab_model.language.domain.holding import BREADBOARD_SURFACE_Z_LAB_MM
...
def normalize_loaded_state(
    ...
    - ``holding`` forced to :func:`lab_model.language.domain.holding.empty_holding` --
    ...
    """
    out = dict(snapshot)
    normalize_components_map(merged_components)
