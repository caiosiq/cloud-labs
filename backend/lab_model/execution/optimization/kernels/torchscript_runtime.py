...
def _repo_root() -> Path:
    # backend/lab_model/execution/optimization/kernels/ â†’ repo root
...
def default_kernels_roots(
    *,
    ...
    schemas/kernels â†’ {lab_view}/kernels â†’ session/job extra roots.
    """
...
def bgr_uint8_to_nchw_float(bgr: Any) -> Any:
    """Convert OpenCV-style HxWxC uint8 BGR â†’ float32 CHW in [0, 1]."""
    import numpy as np
    import torch
    ...
    # BGR â†’ RGB for conventional CHW models
    rgb = arr[:, :, ::-1].copy()
...
def run_torchscript_scalar(
    ...
    *,
    ...
    """Capture-frame â†’ TorchScript ``forward`` â†’ Python float scalar."""
...
def run_torchscript_features(
    ...
    *,
    ...
    """Capture-frame â†’ TorchScript ``forward`` â†’ 1D float feature vector."""
    kind, value = run_torchscript_output(
        kernel_id, bgr, lab_view_path=lab_view_path
    )
    if kind == "scalar":
