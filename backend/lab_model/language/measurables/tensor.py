...
class MeasurableTensor:
    ...
    def to_torch(self, *, dtype: Any = None) -> Any:
        ...
        Call after the tensor is resolved. For images this wraps the
        ``HxWx3 uint8`` BGR array; for scalars/vectors it wraps the inline value.
        ...
        if isinstance(self.data, LazyRef):
            raise ValueError(
                "tensor is not resolved; resolve it before to_torch()"
            )
        try:
