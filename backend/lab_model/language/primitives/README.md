# Lab primitives (`lab_model.language.primitives`)
...
**Import:** `from lab_model.language.primitives import PrimitiveId, parse_command_payload, ...`

Platform map: [`../ARCHITECTURE.md`](../ARCHITECTURE.md). Surfaces/VC: [`../../../docs/LAB_SURFACES_VC_AND_INITIALIZATION.md`](../../../docs/LAB_SURFACES_VC_AND_INITIALIZATION.md).
...
**`MOTOR_SEND_HOME`** (`PrimitiveKind.MACRO`): reads cumulative angle via **`lab_model.language.domain.motor_rotation_store.get_angle`**, then **`await`s** the atomic **`MOVE_MOTOR`** path with `distance = -angle`. It does **not** call `LabCommunicator.motor_send_home` from dispatch (that method remains on the communicator for **direct** callers / parity tests).
