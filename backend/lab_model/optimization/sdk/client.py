...
Example::
    ...
    from lab_model.optimization.sdk import CloudLabsClient

    with CloudLabsClient.connect("mock.default") as lab:
...
class CloudLabsClient:
    ...
    def eval_kernel(
        ...
        *,
        ...
        """Capture a measurable and run a TorchScript kernel → scalar or feature vector.

        Catalog kernels run locally against the captured frame. Session kernels
        (``session.*``) are evaluated via the edge ``/api/kernels/eval`` endpoint
        so the artifact need not exist on the laptop.
        """
        ...
        if not torch_available():
            raise CloudLabsCommandError(
                "PyTorch required for eval_kernel (TorchScript runtime unavailable)"
            )
        tensor = self.measurable(tag_id, field).resolve(record=True)
        kind, value = run_torchscript_output(kid, tensor.data)
        if kind == "features":
            feats = list(value)
            self._vlog("eval_kernel features=%s", feats)
            return feats
    ...
    def kernel_match(
        ...
        *,
        ...
        """One objective term: TorchScript scalar matched to a frozen target."""
