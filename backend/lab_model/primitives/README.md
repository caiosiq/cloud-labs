# Lab primitives (`lab_model.primitives`)
...
**Import:** `from lab_model.primitives import PrimitiveId, parse_command_payload, ...`

UI contract: [`../../../docs/primitive_ui_contract.md`](../../../docs/primitive_ui_contract.md). Platform map: [`../ARCHITECTURE.md`](../ARCHITECTURE.md).
...
**`MOTOR_SEND_HOME`** (`PrimitiveKind.MACRO`): reads cumulative angle via **`lab_model.domain.motor_rotation_store.get_angle`**, then **`await`s** the atomic **`MOVE_MOTOR`** path with `distance = -angle`. It does **not** call `LabCommunicator.motor_send_home` from dispatch (that method remains on the communicator for **direct** callers / parity tests).
