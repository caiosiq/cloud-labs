...
2. **Components as scientific objects.** Each part separates *identity*
   (parameters), *degrees of freedom you set* (tunables), and
...
6. **Backends hub with interpretation.** Live component/kernel reference and
   frozen snapshots per lab — documented with physical meaning, not only tensor
   shapes.
...
| **This Wiki** | Learn curriculum, then **Backends** for live labs |

## Practice

1. Open the **Twin** and select a mirror. Drag the **ghost** to propose a pose,
   confirm the move, and watch the **solid** follow—the same tunable, not a
   measurable.
2. Return here → **Backends** → your lab → the same tag → read **Physical interpretation**.
