/**
 * Snapshots — hierarchical browse of approved pins:
...
const browseLede = document.getElementById('browse-lede');
const publishQueueCard = document.getElementById('publish-queue-card');
...
function scriptSnippetLocal(repoId, branch) {
    return (
        `# Local branch head at load time (not a snapshot pin)\n` +
...
function setBrowseChrome(title, lede) {
    if (browseHeading) browseHeading.textContent = title;
    if (browseLede) browseLede.textContent = lede;
}
...
function renderRepoList() {
    ...
    setBrowseChrome(
        'Choose a repository',
        'Only approved pins appear here. They do not move when you commit locally on Twin.',
    );
    if (!repoIds.length) {
        browseEl.innerHTML =
            '<p class="empty">No approved snapshots yet. Publish a commit from Twin to create the first pin.</p>';
...
async function renderBranchList() {
    ...
    setBrowseChrome(
        'Choose a branch',
        `Branches with pins (and other heads) for ${repoDisplayName(selectedRepoId)}.`,
    );
    browseEl.innerHTML = `
        <div class="pick-grid">
            ${branches
...
function updateGraphHint(nodes) {
    ...
    graphHint.textContent =
        `${pinCount} snapshot pin node(s) on this graph — click a node to preview the bench.`;
}
...
async function renderGraphLevel() {
    setBrowseChrome(
        'Pick a pin on the graph',
        'Blue nodes are selected. Approved snapshot pins can be loaded from scripts; other nodes still preview.',
    );
    browseEl.innerHTML = '';
...
function renderPinDetail(pin, commitId = null) {
    if (!pin && !commitId) {
        ...
        pinSnapshot.textContent =
            'Select an approved snapshot pin to copy script or JSON. Other nodes still preview on the twin.';
        return;
    }
...
function renderRequests(requests) {
    ...
    if (!pending.length) {
        ...
        if (publishQueueCard) publishQueueCard.open = false;
        return;
    }
    requestsEmpty.hidden = true;
    requestsTable.hidden = false;
    if (publishQueueCard) publishQueueCard.open = true;
    ...
    requestsBody.querySelectorAll('button[data-approve]').forEach((btn) => {
        btn.addEventListener('click', async () => {
            const pinId = prompt('Snapshot pin id (optional):', '');
