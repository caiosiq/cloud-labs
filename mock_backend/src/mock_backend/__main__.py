"""``python -m mock_backend`` — serve ``mock_backend/cloudlabs_edge`` on :8100."""
...
def _ensure_edge_on_path() -> Path:
    """Put ``mock_backend/cloudlabs_edge`` first so ``main`` / ``adapters`` resolve."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Cloud Labs mock backend — Edge Contract via cloudlabs_edge/"
    )
