...
- Primitive hook implementations in ``real/`` and ``mock/`` (the
  ...
  ``progress_callback`` -- see ``communicator_refactor.md`` Â§6.2.
"""
...
from lab_model.execution.orchestration.teleop_live_pose import TeleopLivePoseStore
from lab_model.language.domain import motor_rotation_store as motor_rot

from lab_model.language.domain.storage_region import find_storage_slot_and_center

from lab_model.coordinator.catalog.lookup import (
    catalog_wh as _catalog_wh_pure,
    motor_catalog_ok as _motor_catalog_ok_pure,
)
from lab_model.execution.orchestration import (
    TeleopController,
...
from lab_model.coordinator.state.motor_state import (
    inject_motor_rotations_into_state,
    persist_motor_rotations_from_component,
)
from lab_model.coordinator.state.snapshot import (
    LabPose,
    merge_snapshot_components,
    normalize_loaded_state,
)
from lab_model.coordinator.state.runtime_manager import MutationKind, RuntimeManager, default_runtime_state
...
class LabCommunicator:
    ...
    def _on_teleop_motion_idle(self, tag_id: str) -> None:
        with self._state_lock:
            ...
            # Use _teleop_live_get_pose â€” not get_teleop_live_pose (RealLab override
    ...
    def get_catalog(self) -> List[Dict[str, Any]]:
        """Return merged catalog rows (``component_library`` âˆ© ``active_catalog``).
    ...
    def _ensure_fixture_components(self) -> None:
        ...
        from lab_model.coordinator.state.fixture_seed import ensure_fixture_components_in_state
    ...
    def _ensure_overlay_fields_locked(self) -> None:
        ...
        if not isinstance(ll, dict) or not isinstance(ll.get("lines"), list):
            seed: Dict[str, Any] = {}
            try:
                from lab_model.coordinator.backends.lab_view_config import read_laser_lines_doc
    ...
    def get_lab_state(self) -> Dict[str, Any]:
        ...
        - Injects software motor angles into
          ...
          from the motor tracker â€” not a measurable).
    ...
    def return_parameters_for_tag(self, tag_id: str) -> Dict[str, Any]:
        """Static UC parameters (catalog identity). Primitive: ``GET_PARAMETERS``."""
        from lab_model.language.domain.component import get_parameters, set_parameters
        from lab_model.language.parameters import parameters_from_catalog_row

        meta = self._catalog_meta_for_tag(tag_id)
        if isinstance(meta, dict):
            params = parameters_from_catalog_row(meta)
            with self._state_lock:
                comp = (self.current_state.get("components") or {}).get(tag_id)
                if isinstance(comp, dict):
                    set_parameters(comp, params)
            return params
        with self._state_lock:
            comp = (self.current_state.get("components") or {}).get(tag_id)
        return get_parameters(comp) if isinstance(comp, dict) else {}
    ...
    async def eval_kernel_for_tag(
        ...
        *,
        ...
        OPTIMIZE runs kernels in-process via the ensemble backends â€” this
    ...
    def _null_measurables_for_targets(
        ...
        Thin wrapper around
        :func:`lab_model.coordinator.state.commits.null_measurables_for_targets`
        ...
        motion / optimization primitive â€” see
        ``universal_component_architecture.md`` Â§3.2 ("Golden Rule of
    ...
    # ---------------------------------------------------------------
    # Phase 8: per-component TELEOP (delegates to lab_model.execution.orchestration)
    # ---------------------------------------------------------------
    ...
    async def _primitive_end_live_feed(
        ...
        *,
        ...
        from lab_model.coordinator.catalog.schema import resolve_cam_id_for_tag, resolve_telemetry_stream_backend

        backend = resolve_telemetry_stream_backend(catalog_meta or {})
        cam_id = resolve_cam_id_for_tag(catalog_meta or {})
        if backend == "table_cam" and cam_id is not None:
            # Stop streaming only â€” keep recorder TCP session warm for the next
    ...
    def save_session_checkpoint_if_enabled(self) -> None:
        ...
        from mock_backend.shared.session_checkpoint import persist_checkpoint
    ...
    # tightened Stage C lint -- see ``fixing.md`` Â§9 / Phase 2A)
    # ---------------------------------------------------------------
    ...
    def _apply_loaded_pose_to_hardware(
        ...
        Stage C exemption -- see ``fixing.md`` Â§9 item 6 / Phase 2A
        lint tightening).
        """
        return
    ...
    async def move_motor(
        ...
        """Rotate a motor â€” see :func:`lab_model.execution.orchestration.run_move_motor`."""
    ...
    async def set_motor_setpoint(
        ...
        """Commit ``tunables.nominal_motor_positions`` (see ``lab_model.language.tunables.nominal_motor_positions``)."""
        from lab_model.language.tunables import nominal_motor_positions as motor_tunable
    ...
    async def set_exposure_time_ms(self, target_id: str, exposure_time_ms: float) -> None:
        """Commit ``tunables.exposure_time_ms`` (see ``lab_model.language.tunables.exposure_time_ms``)."""
        from lab_model.language.tunables import exposure_time_ms as exposure_tunable
    ...
    async def set_output_power_mw(self, target_id: str, output_power_mw: float) -> None:
        """Commit ``tunables.output_power_mw`` (see ``lab_model.language.tunables.output_power_mw``)."""
        from lab_model.language.tunables import output_power_mw as laser_tunable
    ...
    async def add_component_to_state(self, component_data: Dict[str, Any]) -> None:
        ...
        with self._state_lock:
            ...
            self.current_state["last_updated"] = datetime.now().isoformat()
        self._persist_state()
        from lab_model.language.domain.component import presence_of
    ...
    async def add_component_from_inventory(
        ...
        Writes through :class:`~lab_model.coordinator.state.runtime_manager.RuntimeManager`.
        """
    ...
    #    See ``communicator_refactor.md`` Â§6.2 for the contract.
    #
    ...
    async def optimize_component(
        ...
        """Optimize â€” see :func:`lab_model.execution.orchestration.run_optimize_component`."""
    ...
    async def _primitive_run_ensemble_optimization(
        ...
        *,
        ...
        ``lab_model.execution.optimization.run_ensemble_optimization``.
        """
        raise NotImplementedError
    ...
    def read_camera_bgr(self, tag_id: str):
        ...
        kernels already consume. Returns ``None`` when capture fails â€”
        callers must not invent synthetic frames on real backends.
        """
        from lab_model.language.measurables.capture import read_camera_bgr_for_tag
