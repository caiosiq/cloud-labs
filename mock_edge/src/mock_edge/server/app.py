"""Edge Contract v1 HTTP shell for the mock teaching edge.

Routes are thin: capabilities/bench from files + lab_view, ``POST /execute``
through :func:`mock_edge.dispatch.dispatch_primitive`, streams through
``adapters.live_feed``, teleop WS through ``adapters.teleop``.
"""
...
from mock_edge import contract
from mock_edge.adapters import context, live_feed, teleop
from mock_edge.dispatch import dispatch_primitive
from mock_edge.latch import now_epoch_ms
...
def _load_capabilities(backend_id: str) -> Dict[str, Any]:
    path = _package_root() / "capabilities.json"
    try:
        caps = json.loads(path.read_text(encoding="utf-8"))
    except Exception:  # noqa: BLE001
        try:
            raw = resources.files("mock_edge").joinpath("capabilities.json").read_text(
                encoding="utf-8"
            )
            caps = json.loads(raw)
        except Exception:  # noqa: BLE001
...
def _load_bench(backend_id: str) -> Dict[str, Any]:
    bench_path = _package_root() / "bench" / "layout.json"
...
def create_app(*, lab: Any = None, backend_id: str = "mock.default") -> FastAPI:
    ...
    app = FastAPI(title="cloudlabs-mock-edge", version=contract.CONTRACT_VERSION)
    if lab is not None:
...
def build_default_app() -> FastAPI:
    from mock_edge.bootstrap import bootstrap_host
