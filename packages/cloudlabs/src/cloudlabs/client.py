...
class CloudLabsClient:
    ...
    def probe_kernel(
        ...
        *,
        ...
        """Authoring probe: EVAL_KERNEL primitive (capture + run TorchScript).

        Kernels are **inputs** to this action — not peer lab verbs. Closed-loop
        work uses ``run_optimize`` / ``run_cobyla`` (OPTIMIZE) with the same
        ``kernel_id`` values baked into the objective IR.
        """
        kid = kernel_id.strip()
        lease = self._require_lease()
        self._vlog(
            "probe_kernel (EVAL_KERNEL) tag=%s field=%s kernel=%s",
            tag_id,
            field,
            kid,
        )
        ...
        if str(resp.get("kind") or "") == "features":
            ...
            self._vlog("probe_kernel features=%s", feats)
            return feats
        scalar = float(resp.get("scalar") or 0.0)
        self._vlog("probe_kernel result=%s", scalar)
        return scalar
    ...
    def eval_kernel(
        ...
        *,
        ...
        """Deprecated alias for :meth:`probe_kernel` (EVAL_KERNEL primitive)."""
        warnings.warn(
            "eval_kernel is deprecated; use probe_kernel (EVAL_KERNEL primitive)",
            DeprecationWarning,
            stacklevel=2,
        )
        return self.probe_kernel(tag_id, field, kernel_id=kernel_id)
    ...
    def run_optimize(
        ...
        *,
        ...
        """Submit an ensemble **OPTIMIZE** primitive (closed-loop job).

        Kernels referenced by the objective (or ``kernels=``) are **inputs** to
        OPTIMIZE — package blobs are staged on submit; the edge loop evaluates
        them in-process. Use :meth:`probe_kernel` only for authoring-time preview.
        """
    ...
    def run_cobyla(
        ...
        *,
        ...
        """OPTIMIZE helper: match a TorchScript kernel to a frozen target via COBYLA.

        Builds an ensemble OPTIMIZE; ``match_kernel.kernel_id`` is an **input**
        to that action. Preview values first with :meth:`probe_kernel`.
        """
