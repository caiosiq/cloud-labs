...
Usage (from repo root):
  python scripts/archive/migrate_to_tunables.py schemas/mock_lab_state.json
  python scripts/archive/migrate_to_tunables.py backend/lab_communicator/real/lab_view/default/states/*.json
  python scripts/archive/migrate_to_tunables.py backend/lab_communicator/mock/lab_view/recipes/*.json
"""
...
def main() -> None:
    ...
    if not paths:
        print(
            "Usage: python scripts/archive/migrate_to_tunables.py <file.json> [...]",
            file=sys.stderr,
        )
        sys.exit(1)
    for path in paths:
