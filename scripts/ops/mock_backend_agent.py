#!/usr/bin/env python3
"""Mock backend agent — second process that owns the lab for the coordinator.
...
Terminal 2 (mock backend — start BEFORE submitting work)::

    $env:PYTHONPATH="backend"; python scripts/ops/mock_backend_agent.py
...
_MOCK_SRC = _REPO / "mock_backend" / "src"
for _p in (_BACKEND, _MOCK_SRC):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

_LOG = logging.getLogger("mock_backend_agent")
...
def _bootstrap_mock_lab() -> Any:
    """Prefer ``python -m mock_backend``; this agent remains for poll-attach jobs."""
    ...
    mock_view = _REPO / "mock_backend" / "lab_view"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Cloud Labs mock backend agent (Step B.1)")
    ...
    reg = _post(
        base,
        "/api/edge/register",
        {"backend_id": backend_id, "label": "mock-backend-agent"},
    )
                ...
                if mode == "closed_loop":
                    ...
                    _post(
                        ...
                        f"/api/edge/jobs/{job['job_id']}/complete",
                        {
                            "status": "failed",
                            "error": (
                                "mock backend agent supports closed_loop and "
