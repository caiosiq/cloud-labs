"""Edge Contract v1 HTTP shell for the simulation ``cloudlabs_edge``.
...
WS through ``adapters.teleop``. Physics lives in ``simulation_edge.host``.
"""
...
def create_app(*, lab: Any = None, backend_id: str = "sim.default") -> FastAPI:
    """Build Edge Contract routes over the simulation adapters."""
    app = FastAPI(title="cloudlabs-simulation-edge", version=contract.CONTRACT_VERSION)
    if lab is not None:
