...
class StorageGeometryTests(unittest.TestCase):
    ...
    def test_optical_housing_profile_has_75_mm_base(self):
        ...
        for visual in profile.component_visuals:
            ...
            component_front_axis = rotation.reshape(3, 3) @ np.asarray(
                visual.front_axis_xyz
            )
            yaw = np.deg2rad(COMPONENT_VISUAL_REFERENCE_YAW_DEG)
            ...
            world_front_axis = body_rotation @ component_front_axis
    ...
    def test_generated_scene_contains_compound_collision_geometry(self):
        ...
        self.assertFalse(
            {"tag_9", "tag_15"} & set(scene.spawn_adjustments_mm)
        )
